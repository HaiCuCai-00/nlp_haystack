...
from controller.search import PIPELINE
...
def export_extractive_qa_feedback(
    ...
    for label in labels:
        ...
        if document is None:
            raise HTTPException(
                status_code=500,
                detail="Could not find document with id {label.document_id} for label id {label.id}",
            )
        ...
        if full_document_context:
            ...
            additional_context_at_end = max(
                context_to_add - label.offset_start_in_doc, 0
            )
            end_pos = min(
                label.offset_start_in_doc + len(label.answer) + context_to_add,
                len(text) - 1,
            )
                        ...
                        "qas": [
                            {
                                ...
                                "is_impossible": False,
                                "answers": [
                                    {"text": label.answer, "answer_start": answer_start}
                                ],
                            }
                        ],
                    }
